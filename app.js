...
? `本月优先追 ${productGap || employee.monthUnitsToAverage} 单可补齐该单品均值；按当前净单价测算约 ${employee.monthUnitsToAverage} 单可达本月均值，累计约 ${employee.unitsToAverage || 0} 单可达年度均值`
...
return `<tr><td><strong>${escapeHtml(employee.name)}</strong></td><td class="number">${employee.groups}</td><td class="number">${employee.orders.size}</td><td class="number net-amount">${currency(employee.monthValue)}</td><td class="number net-amount">${currency(employee.value)}</td><td class="group-ai-performance${employee.monthGapToAverage > 0 ? " group-ai-warning" : employee.gapToLeader === 0 ? " group-ai-leading" : ""}">${escapeHtml(performance)}</td><td><strong>${escapeHtml(productName)}</strong>${priority ? `<span>当前 ${plainNumber(priority.current)} 单 · 单月均值 ${plainNumber(priority.averageQuantity, 1)} 单 · 最高 ${plainNumber(priority.maximumQuantity)} 单</span>` : ""}</td><td>${escapeHtml(targetText)}</td><td>${escapeHtml(direction)}</td></tr>`;
  }).join("");
...
elements.groupComparison.innerHTML = `<div class="group-legend"><span><i class="legend-swatch below"></i>低于均值</span><span><i class="legend-swatch leader"></i>年度第一</span><span>产品列显示按订单号去重后的订单数；累计总业绩可查看群级分析建议</span></div><div class="table-wrap"><table class="group-comparison-table" style="min-width:${tableWidth}px"><thead><tr><th>员工姓名</th><th>微信群</th><th class="number">好友数量</th><th class="number">年度订单</th><th class="number">本月业绩</th><th class="number">年度总业绩</th><th class="number">产品总订单数</th><th class="number">开发单品种类</th><th class="number">产品开发占比</th><th class="number">群内开发率</th>${headers}</tr></thead><tbody>${rows}</tbody><tfoot><tr><td></td><td><strong>群均值</strong></td><td></td><td class="number">${plainNumber(report.averageOrders, 1)}</td><td class="number">${currency(averageMonthValue)}</td><td class="number">${currency(report.average)}</td><td class="number">${plainNumber(averageQuantity, 1)}</td><td class="number">${plainNumber(averageTypes, 1)}</td><td class="number">${plainNumber(report.averageProductShare, 1)}%</td><td class="number">${report.averageDevelopmentRate === null ? "-" : `${plainNumber(report.averageDevelopmentRate, 1)}%`}</td>${averages}</tr></tfoot></table></div>`;
  renderGroupEmployeeAiAnalysis(report);
}
